...
head(daily_data_frames_2016)

#save the data
#save.image("regular_season.RData")

#find where the save will be
getwd() #should be /Volumes/files/mlb/savant_daily_data/2016
setwd('/Volumes/files/mlb/savant_daily_data/2016')
getwd()
...
# Inspect the structure of daily_data_frames_2016
print(str(daily_data_frames_2016))
...
standardize_list_item <- function(item) {
  ...
  all_columns <- unique(unlist(lapply(daily_data_frames_2016, names)))
...
daily_data_frames_2016 <- lapply(daily_data_frames_2016, standardize_list_item)

# Combine the standardized list items into a single dataframe
daily_data_frames_2016 <- do.call(rbind, daily_data_frames_2016)
...
conn <- dbConnect(RSQLite::SQLite(), "daily_data_frames_2016.db")

# Write the dataframe to the database
dbWriteTable(conn, "daily_data_frames_2016", daily_data_frames_2016)
