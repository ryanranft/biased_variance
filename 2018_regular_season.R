...
scrape_and_combine_2018 <- function(start_date, end_date, player_type) {
...
# Define the start and end dates for the 2018 season
start_date <- as.Date('2018-03-29')
end_date <- as.Date('2018-10-01')

# Generate a sequence of dates from start_date to end_date
date_ranges_2018 <- lapply(seq(start_date, end_date, by = "day"), function(date) {
  list(start_date = as.character(date), end_date = as.character(date))
})

# Initialize list to store data frames for each day
daily_data_frames_2018 <- list()

# Loop through each date range, scrape data, and store in daily data frames
for (date_range in date_ranges_2018) {
  start_date <- date_range$start_date
  end_date <- date_range$end_date
  
  # Scrape data for current date range
  data_2018 <- scrape_and_combine_2018(start_date = start_date,
                                       end_date = end_date,
                                       player_type = 'batter')
  
  # Store data in a data frame
  daily_data_frames_2018[[start_date]] <- data_2018
}

# Print the first few elements of the list to verify
head(daily_data_frames_2018)

#save the data
#save.image("regular_season.RData")

#find where the save will be
getwd() #should be /Volumes/files/mlb/savant_daily_data/2018
setwd('/Volumes/files/mlb/savant_daily_data/2018')
getwd()
...
# Inspect the structure of daily_data_frames_2018
print(str(daily_data_frames_2018))
...
standardize_list_item <- function(item) {
  ...
  all_columns <- unique(unlist(lapply(daily_data_frames_2018, names)))
...
daily_data_frames_2018 <- lapply(daily_data_frames_2018, standardize_list_item)

# Combine the standardized list items into a single dataframe
daily_data_frames_2018 <- do.call(rbind, daily_data_frames_2018)
...
conn <- dbConnect(RSQLite::SQLite(), "daily_data_frames_2018.db")

# Write the dataframe to the database
dbWriteTable(conn, "daily_data_frames_2018", daily_data_frames_2018)
