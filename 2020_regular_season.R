...
scrape_and_combine_2020 <- function(start_date, end_date, player_type) {
...
# Define the start and end dates for the 2020 season
start_date <- as.Date('2020-07-23')
end_date <- as.Date('2020-09-27')

# Generate a sequence of dates from start_date to end_date
date_ranges_2020 <- lapply(seq(start_date, end_date, by = "day"), function(date) {
  list(start_date = as.character(date), end_date = as.character(date))
})

# Initialize list to store data frames for each day
daily_data_frames_2020 <- list()

# Loop through each date range, scrape data, and store in daily data frames
for (date_range in date_ranges_2020) {
  start_date <- date_range$start_date
  end_date <- date_range$end_date
  
  # Scrape data for current date range
  data_2020 <- scrape_and_combine_2020(start_date = start_date,
                                       end_date = end_date,
                                       player_type = 'batter')
  
  # Store data in a data frame
  daily_data_frames_2020[[start_date]] <- data_2020
}

# Print the first few elements of the list to verify
head(daily_data_frames_2020)

#find where the save will be
getwd() #should be /Volumes/files/mlb/savant_daily_data/2020

#if not in proper wd
setwd("/Volumes/files/mlb/savant_daily_data/2020")
#getwd()
...
# Inspect the structure of daily_data_frames_2020
print(str(daily_data_frames_2020))
...
standardize_list_item <- function(item) {
  ...
  all_columns <- unique(unlist(lapply(daily_data_frames_2020, names)))
...
daily_data_frames_2020 <- lapply(daily_data_frames_2020, standardize_list_item)

# Combine the standardized list items into a single dataframe
daily_data_frames_2020 <- do.call(rbind, daily_data_frames_2020)
...
conn <- dbConnect(RSQLite::SQLite(), "daily_data_frames_2020.db")

# Write the dataframe to the database
dbWriteTable(conn, "daily_data_frames_2020", daily_data_frames_2020)
