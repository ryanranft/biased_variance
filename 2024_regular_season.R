...
start_date <- as.Date('2024-03-20', tz = "America/Chicago")
end_date <- as.Date('2024-09-29', tz = "America/Chicago")  # Adjust the end date as needed
...
# Convert the columns in the scraped data to the specified types
all_data <- convert_to_specified_types(all_data, column_types)

# Print the final data with the specified types
print(all_data)

# Define the file paths for saving the CSV and DB files
csv_file_path <- "/Volumes/files/data/mlb/savant/2024/pbp_regular_season_2024.csv"
db_file_path <- "/Volumes/files/data/mlb/savant/2024/pbp_regular_season_2024.db"
...
save.image("/Volumes/files/data/mlb/savant/2024/pbp_regular_season_2024.RData")
