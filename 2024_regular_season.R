...
scrape_and_combine_2024 <- function(start_date, end_date, player_type) {
...
# Inspect the structure of daily_data_frames_2024
print(str(daily_data_frames_2024))
...
standardize_list_item <- function(item) {
  ...
  all_columns <- unique(unlist(lapply(daily_data_frames_2024, names)))
...
daily_data_frames_2024 <- lapply(daily_data_frames_2024, standardize_list_item)

# Combine the standardized list items into a single dataframe
daily_data_frames_2024 <- do.call(rbind, daily_data_frames_2024)
...
conn <- dbConnect(RSQLite::SQLite(), "daily_data_frames_2024.db")

# Write the dataframe to the database
dbWriteTable(conn, "daily_data_frames_2024", daily_data_frames_2024)
